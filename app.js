...
const ourForEach = (arr, action) => {
  for(let i= 0; i < arr.length; i++){
    action(arr[i])
  }
}

ourForEach(game.party, (partyPokemon) => {
...
for (let pokemon of game.pokemon) {
  if (pokemon.starter === true) {
      starterPokemon.push(pokemon.name);
  }
}

  console.log('Exercise 10 Results:');
...
game.catchPokemon = function(pokemonObj) {
  this.party.push(pokemonObj)
  const pokeballIndex = this.items.findIndex(item.name === "pokeball");
  if (pokeballIndex !== -1) {
